...
public class TransparentBackgroundScreenshotRecorder : MonoBehaviour
{
    ...
    void SavePng()
    {
        int filesCount = System.IO.Directory.GetFiles(folderName).Length;
        Debug.Log(filesCount);
        string name = string.Format("{0}/" + (filesCount + 1).ToString() + fileName + ".png", folderName, videoFrame);
        //string name = string.Format("{0}/" + (filesCount + 1).ToString() + " shot.png", folderName, videoFrame);
